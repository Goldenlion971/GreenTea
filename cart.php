<?php 
 ...
 if (isset($_SESSION['user_id'])) {
	...
	if (isset($_POST['update_cart'])) {
		...
		$success_msg[] = 'la quantité du panier a été mise à jour avec succès';
	}
	...
	if (isset($_POST['delete_item'])) {
		...
		if ($varify_delete_items->rowCount()>0) {
			...
			$success_msg[] = "article du panier supprimé avec succès";
		}else{
			$warning_msg[] = 'article du panier déjà supprimé';
		}
	}
...
<head>
	...
	<title>Green Coffee - favoris</title>
</head>
<body>
		...
		<section class="products">
			...
			<div class="box-container">
				...
				<form method="post" action="" class="box">
					...
					<p class="sub-total">total : <span>$<?=$sub_total = ($fetch_cart['qty']* $fetch_cart['price']) ?></span></p>
			...
			<div class="cart-total">
				<p>montant total à payer : <span>€ <?= $grand_total; ?></span></p>
				<div class="button">
					<form method="post">
						<button type="submit" name="empty_cart" class="btn" onclick="return confirm('êtes vous sûr de vouloir vider votre panier ?')">annuler</button>
					</form>
